...
class CarrierService extends Service
{
    ...
    */
    ...
    public function datatable($request)
    {
        try {
            ...
            if (empty($request->input('search.value'))) {
                ...
                $shipperLists = Carrier::select('*')->search($searchKey);
                $totalShipperCount = $shipperLists->count();
                $shippers = $shipperLists->offset($start)
                    ->limit($limit)
                    ->orderBy($order, $dir)
    ...
    {
        try {
            $data = Carrier::select('id', 'name', 'active_status')->get();
