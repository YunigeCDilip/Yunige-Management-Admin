...
class MovementConfirmationService extends Service
{
    /**
     *
    ...
    */
    ...
    public function datatable($request)
    {
        try {
            ...
            if (empty($request->input('search.value'))) {
                $shipperLists = MovementConfirmation::select('*');
                ...
                $shipperLists = MovementConfirmation::select('*')->search($searchKey);
                $totalShipperCount = $shipperLists->count();
                $shippers = $shipperLists->offset($start)
                    ->limit($limit)
                    ->orderBy($order, $dir)
    ...
    {
        try {
            $data = MovementConfirmation::select('id', 'name', 'active_status')->get();
    ...
    {
        try {
            $data = MovementConfirmation::find($id);
            if(!$data){
    ...
    public function destroy($request, $id)
    {
        try {
            $this->db->beginTransaction();
            $category = MovementConfirmation::find($id);
            $isShipperUsed = Client::where('movement_confirmation_id', $category->id)->count();
            if($isShipperUsed > 0){
    ...
    {
        try {
            $user = MovementConfirmation::find($request->id);
