...
use App\Http\Resources\ItemLabelResource;

class ItemLabelService extends Service
{
    /**
     *
    ...
    */
    ...
    {
        try {
            $data = QueryBuilder::for(ItemLabel::Search(request('search')))
                ->defaultSort('name')
                ->allowedSorts('id', 'name')
               ->paginate((request('per_page')) ?? 20);
            return $this->responsePaginate(ItemLabelResource::collection($data), MessageResponse::DATA_LOADED);
        } catch (Throwable $e) {
    ...
    public function datatable($request)
    {
        try {
            ...
            if (empty($request->input('search.value'))) {
                ...
                $shipperLists = ItemLabel::select('*')->search($searchKey);
                $totalShipperCount = $shipperLists->count();
                $shippers = $shipperLists->offset($start)
                    ->limit($limit)
                    ->orderBy($order, $dir)
    ...
    {
        try {
            $data = ItemLabel::select('id', 'name')->get();
