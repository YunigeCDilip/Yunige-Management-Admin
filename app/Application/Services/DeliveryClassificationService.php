...
class DeliveryClassificationService extends Service
{
    /**
     *
    ...
    */
    ...
    public function datatable($request)
    {
        try {
            ...
            if (empty($request->input('search.value'))) {
                $shipperLists = ForeignDeliveryClassification::select('*');
                ...
                $shipperLists = ForeignDeliveryClassification::select('*')->search($searchKey);
                $totalShipperCount = $shipperLists->count();
                $shippers = $shipperLists->offset($start)
                    ->limit($limit)
                    ->orderBy($order, $dir)
    ...
    {
        try {
            $data = ForeignDeliveryClassification::select('id', 'name', 'active_status')->get();
    ...
    public function store($request)
    {
        try {
            ...
            $data = new ForeignDeliveryClassification();
    ...
    {
        try {
            $data = ForeignDeliveryClassification::find($id);
            if(!$data){
    ...
    public function destroy($request, $id)
    {
        try {
            $this->db->beginTransaction();
            $category = ForeignDeliveryClassification::find($id);
            $isShipperUsed = Client::where('foreign_delivery_classifications_id', $category->id)->count();
            if($isShipperUsed > 0){
    ...
    {
        try {
            $user = ForeignDeliveryClassification::find($request->id);
