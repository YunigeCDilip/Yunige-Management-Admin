...
class WdataPicService extends Service
{
    ...
    */
    ...
    public function datatable($request)
    {
        try {
            ...
            if (empty($request->input('search.value'))) {
                ...
                $shipperLists = WdataPic::select('*')->search($searchKey);
                $totalShipperCount = $shipperLists->count();
                $shippers = $shipperLists->offset($start)
                    ->limit($limit)
                    ->orderBy($order, $dir)
    ...
    {
        try {
            $data = WdataPic::select('id', 'name', 'active_status')->get();
