...
jQuery(document).ready(function(){
    ...
    $('table#table').delegate('.mdi-square-edit-outline', 'click', function(e){
        ...
        $.get(baseUrl+"inbound-statuses/"+shipperId, function(data){
    ...
    $('.update-shipper').on('click', function(e){
        ...
        $.ajax({
            type: "PUT",
            url: baseUrl + "inbound-statuses/"+$('form#editForm').find('input[name="shipper_id"]').val(),
            data: form_data,
            success: function (data) {
...
$('table#table').delegate('.mdi-delete', 'click', function(e){
    ...
    swal({
        ...
        if(isConfirm){
            $.ajax({
                url: baseUrl + 'inbound-statuses/' + userId,
                type: 'post',
