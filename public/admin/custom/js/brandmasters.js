...
$(function(){
    $(".select2").select2();
    $(".logo_attachment").dropify({
        messages:{
            default:"Drag and drop a logo here",
...
$(function(){
    table = $("#table").DataTable({
        ...
        columns: [
            {data: 'name'},
            {data: 'brand_logo', render: function(data, type, dataObject, meta){
                if(data != '' && data != null){
                    return '<a href="'+data+'" target="_blank"><img src="'+data+'" alt="product-img" height="32"></a>'
                }else{
                    return '-';
                }
            }},
            ...
            {data: 'actions', searchable: false, orderable: false, sortable: false,
                render: function(data, type, dataObject, meta) {
                    var action = '';
                    // action += '<a href="'+baseUrl+'item-brands/'+dataObject.id+'" class="action-icon"> <i class="mdi mdi-eye text-success"></i></a>';
                    action += '<a href="'+baseUrl+'item-brands/'+dataObject.id+'/edit" class="action-icon"> <i class="mdi mdi-square-edit-outline text-primary"></i></a>';
...
$('.save-data').on('click', function(e){
    ...
    var logo;
    addForm.find('.logo-file').each(function(key, value){
        logo = $(this).find('input[name="logo"]')[0].files[0];
        (typeof logo == 'undefined') ? '' : form_data.append('logo', logo);
    });
...
$(document).on("click",".update-data", function (e){
    ...
    var Id = editForm.find('input[name="brand_id"]').val();
    $(this).prop('disabled', true);
    $(this).parents('.form-group').find('.spinner-border').show();
    var thisReference = $(this);
    var form_data = new FormData();
    var logo;
    editForm.find('.logo-file').each(function(key, value){
        logo = $(this).find('input[name="logo"]')[0].files[0];
        (typeof logo == 'undefined') ? '' : form_data.append('logo', logo);
    });
...
$('table#table').delegate('.mdi-delete', 'click', function(e){
    ...
    swal({
        ...
        if(isConfirm){
            $.ajax({
                url: baseUrl + 'item-brands/' + clientId,
                type: 'post',
