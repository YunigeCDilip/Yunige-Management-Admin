        ...
        columns: [
            ...
            {data: 'actions', searchable: false, orderable: false, sortable: false,
                render: function(data, type, dataObject, meta) {
                    var action = '';
                    action += '<a href="'+baseUrl+'fba/'+dataObject.id+'/edit" class="action-icon"> <i class="mdi mdi-square-edit-outline text-primary"></i></a>';
...
$('.update-fba').on('click', function(e){
    e.preventDefault();
...
$('table#table').delegate('.mdi-delete', 'click', function(e){
    ...
    swal({
        ...
        if(isConfirm){
            $.ajax({
                url: baseUrl + 'fba/' + fba_Id,
                type: 'post',
                data:{ id:fba_Id, _method: 'DELETE', _token: csrfToken
                },
                success: function(data){
                    table.draw();
                    messages(data.message, data.status);
